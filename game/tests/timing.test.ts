...
import { animationCadenceForState, GameState } from '../src/core/GameState';
...
function countFixedFrames(
  refreshRate: number,
  seconds: number,
  stepMs: number,
  maxSteps: number,
): number {
  const clock = new FixedStepClock(stepMs);
  const renderFrames = Math.round(refreshRate * seconds);
  const elapsedMs = 1000 / refreshRate;
  let frames = 0;
  for (let i = 0; i < renderFrames; i++) {
    frames += clock.advance(elapsedMs, 1, maxSteps);
  }
  return frames;
}
...
test('H5重制过场固定约60FPS推进，不会被10Hz原版界面节拍放慢', () => {
  for (const hz of [60, 120, 144]) {
    assert.equal(countFixedFrames(hz, 1, 1000 / 60, 6), 60);
  }
});

test('页面后台停留时间不会在恢复首帧进入真实时间计时', () => {
  const clock = new VisibilityAwareClock(1000, true);
  assert.equal(clock.advance(1250), 250);
  clock.setVisible(false, 1300);
  assert.equal(clock.advance(20_000), 0);
  clock.setVisible(true, 20_000);
  assert.equal(clock.advance(20_120), 120);
});

test('原版菜单与军议背景在高刷屏上仍严格按10Hz推进', () => {
  for (const hz of [60, 120, 144]) {
    assert.equal(countFixedFrames(hz, 10, 100, 10), 100);
  }
});

test('非战斗页面明确区分原版界面节拍与H5连续动画节拍', () => {
  const originalUiStates = [
  ...
  const cinematicStates = [
    GameState.LOADING_SCREEN,
    GameState.LEVEL_INTRO,
    GameState.ENDING_ANIM,
  ];

  for (const state of originalUiStates) assert.equal(animationCadenceForState(state), 'original-ui');
  for (const state of cinematicStates) assert.equal(animationCadenceForState(state), 'cinematic');
  assert.equal(animationCadenceForState(GameState.PLAYING), null);
});
...
test('桌面出兵按钮同步显示并关闭连战倒计时', () => {
  const ui = new UISystem({} as never);
  let waveLabel = '';
  ui.setButtonCallback((button) => {
    if (button.action === 'start_wave') waveLabel = button.label;
  });
  ui.setWaveReady(true);
  ui.setComboCountdownSeconds(4.8);
  assert.equal(ui.handleTap(180, 30), true);
  assert.equal(waveLabel, '4.8s 出兵');

  ui.setComboCountdownSeconds(null);
  ui.handleTap(180, 30);
  assert.equal(waveLabel, '出兵 ▶');
});
...
test('断龙闸释放期间重新开栏仍保留机关动作首项，不会把确认映射成拆除', () => {
  const tower = createTower(10, { gateState: 1, gateLoaded: false });
  let loads = 0;
  let upgrades = 0;
  let demolishes = 0;
  const bar = new BuildBarSystem({} as never);
...
test('触屏建筑已选中后单击升级一次，并在关闭动画期间防止重复提交', () => {
  const tower = createTower(1);
  let upgrades = 0;
  const bar = new BuildBarSystem({} as never);
  bar.setHost({
...
test('触屏升级改为单击后，拆除仍必须点击同一项目两次', () => {
  ...
  bar.handleTap(120, 220); // 第一次点拆除，只选中
  assert.equal(upgrades, 0);
  assert.equal(demolishes, 0);
...
test('军议科技折扣按优惠价扣费，且仅在装置成功建成后消费', () => {
  const council = new CouncilSystem(new SeededRng(1), {
    techDiscounts: [0.4, 0, 0, 0, 0],
  });
  let spent = -1;
  const bar = new BuildBarSystem({} as never);
  bar.setHost({
    getGold: () => 999,
    trySpendGold: cost => { spent = cost; return true; },
    getTowerCount: () => 1,
    getTechCostDetail: techIndex => {
      const baseCost = TECH_COST_G1057[techIndex];
      const discount = council.techDiscount(techIndex);
      return {
        baseCost,
        discount,
        finalCost: Math.round(baseCost * (1 - discount)),
        explanations: [],
      };
    },
    enterPlacement() {},
    ...
    renderTowerPreview() {},
    deselectTower() {},
    onTechBuilt: techIndex => { council.consumeTechDiscount(techIndex); },
  });
  bar.setTouchOptimized(true);
  bar.open(BAR_CAT_TECH);
  bar.update(1000);
  ...
  bar.handleTap(40, 220);
  assert.equal(council.techDiscount(0), 0.4);
  bar.handleTap(40, 220);
  assert.equal(spent, 12);
  assert.equal(council.techDiscount(0), 0);
});
...
function audioVoiceHarness(currentTime: number = 0) {
  const starts: Array<{ source: Record<string, unknown>; when: number }> = [];
  const stops: number[] = [];
  const ramps: unknown[][] = [];
  ...
  const createSource = () => {
    const source: Record<string, unknown> = {
      ...
      onended: null,
      connect: () => {},
      disconnect: () => { sourceDisconnects++; },
      start: (when: number) => { starts.push({ source, when }); },
  ...
  const createGain = () => ({
    gain: {
      value: 1,
      setValueAtTime: (...args: unknown[]) => ramps.push(['set', ...args]),
      exponentialRampToValueAtTime: (...args: unknown[]) => ramps.push(['attack', ...args]),
      cancelAndHoldAtTime: (...args: unknown[]) => ramps.push(['hold', ...args]),
      linearRampToValueAtTime: (...args: unknown[]) => ramps.push(['release', ...args]),
    },
    connect: () => {},
    disconnect: () => { gainDisconnects++; },
  });
  const context = {
    currentTime,
    state: 'running',
  ...
  return {
    ...
    stops,
    ramps,
    get sourceDisconnects() { return sourceDisconnects; },
    get gainDisconnects() { return gainDisconnects; },
  };
}

test('SoundFont 采样使用 AudioContext 当前时间启动', () => {
  const harness = audioVoiceHarness(12.5);
  const sample = { id: 'A4' };
  const audio = new AudioSystem();
  Object.assign(audio as unknown as Record<string, unknown>, {
    audioContext: harness.context,
    masterGain: harness.masterGain,
    soundfontBuffers: { 69: sample },
    useSoundfontForPlayback: true,
    volume: 0.5,
  });
  ...
  handleMidi.call(audio, { name: 'Note on', channel: 1, noteNumber: 69, velocity: 100 });
  assert.equal(harness.starts.length, 1);
  assert.equal(harness.starts[0].when, 12.5);
  assert.equal(harness.starts[0].source.buffer, sample);
});

test('SoundFont 超范围音符按八度折回可用钢琴采样区间', () => {
  const harness = audioVoiceHarness(4);
  const highSample = { id: 'high' };
  const lowSample = { id: 'low' };
  const audio = new AudioSystem();
  Object.assign(audio as unknown as Record<string, unknown>, {
    audioContext: harness.context,
    masterGain: harness.masterGain,
    soundfontBuffers: { 101: highSample, 32: lowSample },
    useSoundfontForPlayback: true,
    volume: 1,
  });
  const handleMidi = (audio as unknown as {
  ...
  handleMidi.call(audio, { name: 'Note on', channel: 1, noteNumber: 113, velocity: 127 });
  handleMidi.call(audio, { name: 'Note on', channel: 2, noteNumber: 8, velocity: 127 });
  assert.deepEqual(harness.starts.map(call => call.source.buffer), [highSample, lowSample]);
});

test('MIDI Note off 淡出音符并在节点结束后彻底回收', () => {
  const harness = audioVoiceHarness(7);
  const audio = new AudioSystem();
  Object.assign(audio as unknown as Record<string, unknown>, {
    audioContext: harness.context,
    masterGain: harness.masterGain,
    soundfontBuffers: { 69: { id: 'A4' } },
    useSoundfontForPlayback: true,
  });
  const internals = audio as unknown as {
    activeVoices: Set<unknown>;
    voicesByKey: Map<string, unknown>;
    handleMidiEvent(event: unknown): void;
  };

  internals.handleMidiEvent({ name: 'Note on', channel: 1, noteNumber: 69, velocity: 100 });
  assert.equal(internals.activeVoices.size, 1);
  assert.equal(internals.voicesByKey.size, 1);

  internals.handleMidiEvent({ name: 'Note off', channel: 1, noteNumber: 69, velocity: 0 });
  assert.equal(internals.voicesByKey.size, 0);
  assert.ok(Math.abs(harness.stops[0] - 7.12) < 1e-9);
  assert.ok(harness.ramps.some(call => call[0] === 'release'));

  const source = harness.starts[0].source as { onended: (() => void) | null };
  source.onended?.();
  assert.equal(internals.activeVoices.size, 0);
  assert.equal(harness.sourceDisconnects, 1);
  assert.equal(harness.gainDisconnects, 1);
});

test('BGM 长时间重复发声不会累积已经结束的音频节点', () => {
  const harness = audioVoiceHarness(3);
  const audio = new AudioSystem();
  Object.assign(audio as unknown as Record<string, unknown>, {
    audioContext: harness.context,
    masterGain: harness.masterGain,
  ...
  for (let i = 0; i < 500; i++) {
    internals.handleMidiEvent({ name: 'Note on', channel: 1, noteNumber: 69, velocity: 100 });
    internals.handleMidiEvent({ name: 'Note off', channel: 1, noteNumber: 69, velocity: 0 });
    const source = harness.starts[i].source as { onended: (() => void) | null };
    source.onended?.();
  }

  assert.equal(internals.activeVoices.size, 0);
  assert.equal(internals.voicesByKey.size, 0);
  assert.equal(harness.sourceDisconnects, 500);
  assert.equal(harness.gainDisconnects, 500);
});

test('General MIDI 打击乐通道不会被错误播放成钢琴噪声', () => {
  const harness = audioVoiceHarness(2);
  const audio = new AudioSystem();
  Object.assign(audio as unknown as Record<string, unknown>, {
    audioContext: harness.context,
    masterGain: harness.masterGain,
    soundfontBuffers: { 45: { id: 'sample' } },
    useSoundfontForPlayback: true,
  });
  const handleMidi = (audio as unknown as {
    handleMidiEvent(event: unknown): void;
  ...
  handleMidi.call(audio, { name: 'Note on', channel: 10, noteNumber: 45, velocity: 127 });
  assert.equal(harness.starts.length, 0);
});

test('BGM 播放完会循环，手动停止后不会被延迟回调重新启动', async () => {
  let plays = 0;
  const audio = new AudioSystem();
  Object.assign(audio as unknown as Record<string, unknown>, {
    midiPlayer: { play: () => { plays++; }, stop: () => {} },
    currentTrack: './mid/0.mid',
    isPlaying: true,
    playbackGeneration: 3,
  });
  const handleEnd = (audio as unknown as { handleEndOfFile(): void }).handleEndOfFile;

  handleEnd.call(audio);
  ...
  assert.equal(audio.isAudioPlaying, false);
});

test('音量设置作用于统一主增益而不是放大每个并发音符', () => {
  ...
  Object.assign(audio as unknown as Record<string, unknown>, {
    ...
    masterGain: {
      gain: {
        cancelScheduledValues: (...args: unknown[]) => changes.push(['cancel', ...args]),
        setTargetAtTime: (...args: unknown[]) => changes.push(['target', ...args]),
      },
    },
  });
  audio.setVolume(0.8);
  assert.deepEqual(changes[0], ['cancel', 7]);
  assert.equal(changes[1][0], 'target');
  assert.ok(Math.abs(Number(changes[1][1]) - 0.56) < 1e-9);
  assert.deepEqual(changes[1].slice(2), [7, 0.015]);
});
