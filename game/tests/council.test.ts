...
test('状态军议不再要求先建对应建筑，油料充足只要求已有烟火', () => {
  for (const id of ['long_burn', 'deep_freeze', 'toxic_lime', 'overcharge']) {
    const def = COUNCIL_BY_ID.get(id)!;
    assert.equal(def.requiresBuiltAny, undefined);
    assert.equal(def.requiresBuiltAll, undefined);
  }

  const selectedIds = ['long_burn', 'deep_freeze', 'toxic_lime', 'overcharge', 'forced_march'];
  const withoutSmoke = new CouncilSystem(new SeededRng(11), { selectedIds });
  assert.equal(withoutSmoke.offerAfterWave(4, 12).some(item => item.id === 'oil_reserve'), false);
  const withSmoke = new CouncilSystem(new SeededRng(11), { selectedIds });
  assert.equal(withSmoke.offerAfterWave(4, 12, { builtTowerTypes: [3] }).some(item => item.id === 'oil_reserve'), true);
});

test('选择状态军议立即获得对应科技折扣和一座低价样机', () => {
  const council = new CouncilSystem(new SeededRng(1), {
    pendingWave: 4,
    pendingOffer: ['deep_freeze'],
  });
  council.resolve('deep_freeze');

  assert.equal(council.techDiscount(4), 0.4);
  assert.equal(council.prototypeDiscount(5), 0.5);
  assert.equal(council.prototypeDiscount(4), 0);
  assert.equal(council.consumeTechDiscount(4), true);
  assert.equal(council.techDiscount(4), 0);
  assert.equal(council.consumePrototypeDiscount(5), true);
  assert.equal(council.prototypeDiscount(5), 0);
});

test('旧存档缺少配套折扣字段时补零，并约束异常折扣范围', () => {
  const legacy = new CouncilSystem(new SeededRng(1), { selectedIds: ['deep_freeze'] });
  assert.deepEqual(legacy.state.techDiscounts, [0, 0, 0, 0, 0]);
  assert.equal(legacy.state.prototypeDiscounts.length, 11);
  assert.equal(legacy.state.prototypeDiscounts.every(value => value === 0), true);

  const normalized = new CouncilSystem(new SeededRng(1), {
    techDiscounts: [2, -1, Number.NaN],
    prototypeDiscounts: [0.25],
  });
  assert.deepEqual(normalized.state.techDiscounts, [0.95, 0, 0, 0, 0]);
  assert.deepEqual(normalized.state.prototypeDiscounts.slice(0, 2), [0.25, 0]);
});
...
test('经典规则不会应用强化规则的样机折扣', () => {
  const council = new CouncilSystem(new SeededRng(1), { prototypeDiscounts: [0, 0.5] });
  const resolver = new ModifierResolver(new FeatureContext('classic'), council);
  assert.equal(resolver.prototypeTowerDiscount(1), 0);
  assert.equal(resolver.buildCost(30, 1), 30);
});
