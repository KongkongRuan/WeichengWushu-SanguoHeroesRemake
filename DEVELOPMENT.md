...
> **2026-07-20 重大更新**：本节以下内容已部分过时。本轮基于 vineflower 反编译 + 字节码验证完成了两项大修：
> 1. **战场系统重写**：证实 mapsp 为纯装饰层，真正路径在 mapdata 属性层（E1163：0/2/4/6=路径方向、8=可建造、10=敌城、11=我城），已导出 `terrainLayer` 并入 level JSON；敌人移动改为原版方向链（10FPS、格中心换向、占位标记）；刷怪改为原版波次制（i1079 兵种表/j1080 概率/l1072 波数/每4波名将波 HP×10/出生格占用门控）；城墙按 /s 图集 e1058+f1063 部件表拼装渲染；修复了漏怪软锁死（过关改波次完成制）、城防 20→10、敌人类型写死 3 的问题。T1/T2/T3/T4 已完成。
> 2. **游戏流程重写**：删除自造菜单，还原原版流程——sflogo 开场 → ld 加载屏 → 声音询问 → 标题(mu 龙logo动画+back[25] 7项转盘：王者之路/自由模式/继续游戏/设置/帮助/关于/退出) → 选择国家(back[7]地图+三国标记+君主画像) → 升级模式(武/文系) → 选择战役(9战役点+敌将面板) → 剧情卷轴 → 战斗；新增软键条(ui[2])；d1073 战役解锁推进与 X/ac/T/aO 入存档。M1 已完成。
>
> 本轮新增资源：mu/sflogo/ld/s/e1-e5/h/eff/end/t1-t10/sp0-3 图包（extract_packs.py 切分，sprite_list.json 160→238 条）。新增数据表 20 余个（i1079/j1080/l1072/a1067/z1131/h1074/A1132/y1130/l1083/e1058/f1063/a1028/b1030/n1076/m1075/k1068/d1091/a1092 等，见 gameData.ts 注释）。
>
> 已知简化点（代码内均有注释）：4向绕行简化为原地等待、我城部件不过滤 b1059、名将波登场横幅简化、自由模式为9战全可打（与原版 h1074 循环有出入）、sflogo 动画简化为4帧循环、结算屏未移植 end 图集动画、4向绕行搜索未实现。下表其余项目（M4 特效、M5 UI贴图、L2 音效等）仍待做。
