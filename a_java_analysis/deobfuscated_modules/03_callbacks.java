...
//    2. 通过 DirectUtils 获取 DirectGraphics 存入 a1005
//    3. 设置字体为 a (gameFont)
//    4. 调用 A() — 主渲染分发器
// ============================================================================
...
//
//    状态机流程:
//    1. 检查 B1171 (已有线程标志), 若!=false 则调用 c() 等待并返回
//    2. 设置 B1171=true, 创建同步锁对象 a1172
//    3. while(true) 主循环:
//       a. 检查 b1018(暂停标志), 若为false则调用 CMidlet.a() 退出
//       b. 记录帧开始时间
//       c. 调用 G() — 主帧更新逻辑(状态分发)
//       d. 清除按键状态 h1047=0
...
//
//    注意: VF L27351 中 B1171 是 boolean 类型, 不是 int
//          v1 版本错误地写成了 int 比较
// ============================================================================
public final void run() {
    // 检查是否已有线程在运行
...
public final void keyPressed(int keyCode) {
    ...
    try {
        ...
        //     if (l == 46) {
        //         if (h1047 != -5) {
        //             if (h1047 != -7) break; // 跳过 o=-1
        //         }
        //     }
        //     o = -1;
        //   即: o=-1 当 l!=46 或 h1047==-5 或 h1047==-7
        //   Procyon确认: if (l != 46 || h1047 == -5 || h1047 == -7) o = -1;
        //   
...
//
//    根据 cb 值决定恢复模式:
//    - cb==1000: 设置 b1174=2 (完全恢复)
//    - C1175==true: 设置 b1174=1 (部分恢复)
//    - 其他: 设置 b1174=3 (默认恢复)
//    最后 cb=0
// ============================================================================
public final void showNotify() {
...
//
//    流程:
//    1. 若 cb==1500: 设置 C1175=true (标记需要恢复)
//    2. 检查 l(游戏状态), 仅在以下状态处理隐藏:
//       l==2, 12, 13, 22, 23 (执行a(3))
//       l==46, 47, 48 → 不处理, 直接返回
//       (其他状态也直接返回)
//    3. 调用 a(3) — 菜单跳转到状态3
//    4. 若 l1031(暂停标志): 调用 h(aq) — 恢复音频
