...
// a(int) — 瓦片属性 E1163[n]>>1 (VF L9997-10001)
...
//    从 s1102/r1101/u1104 数组计算 HP 值
//    ★ v9 修正: 完整恢复 (原为存根)
//
//    逻辑:
//    1. 若 var2==s1102 (备用HP数据):
//       - 检查 f1106[0], true则bonus=5, false则bonus=0
//       - 调用 i(var1,var3), 若返回非0:
//         bonus += d(var2[var1<<1], var2[(var1<<1)+1], var3) >> 2
//    2. 否则若 var2==u1104 (塔HP数据):
...
private int a(int var1, byte[] var2, int var3) {
    int bonus = 0;
    byte[] sData = this.s1102;
    int var19;

    if (var2 == sData) {
        // 使用 s1102 (备用HP数据)
        boolean[] flags = this.f1106;
        if (flags[0]) {
            var19 = 5;
        } else {
            var19 = 0;
        ...
        // 检查是否为 u1104 (塔HP数据)
        byte[] uData = this.u1104;
        if (var2 == uData) {
            boolean isTower = this.n(var1, var3);
            if (isTower) {
                var19 = -10;
            } else {
                var19 = 0;
    ...
    int baseHP = d(baseHi, baseLo, var3);
...
private static short a(InputStream var0) throws IOException {
...
private void a(int var1, int var2, int var3, int var4, int var5) {
    if (this.o != -1) {
        ...
        String text = this.b1015[var1 + 113];
        // ... 绘制文字面板
    }
}
...
//    若 D1176=true, 添加代理前缀 "http://10.0.0.172:80"
...
private boolean b(int var1) {
    ...
    if (var2) {
        if (this.E1163[var1] < 8) {
            if (this.D1162[var1] < 6) {
                return true;
            }
        }
    }
    this.m(var1);  // ★ 不可建造时调用 m(var1) (VF L16862)
    return false;
}
...
private boolean b(int var1, int var2) {
    ...
    if (var4) {
        if (this.E1163[var3] < 8) {
            if (this.D1162[var3] < 6) {
                return true;
            }
        }
    }
    this.m(var3);  // 不可建造时调用 m(var3) (VF L16885)
    return false;
}
...
private void c(int var1) {
    if (this.a1013[var1] == null) {
        // 从 a1014[var1] 路径加载精灵图
        // ... 完整见 VF L16985
    }
}
...
//      5. 响应体读取并hex解码到 I1189
//      6. 连接清理: close + a1193=null
...
private int d(int var1) {
    int var4 = this.E1163[var1];
    if (var4 >= 12) {
        var4 = this.E1163[var1] - 12;
    } else if (var4 <= -12) {
        var4 = 98 - this.E1163[var1];
    } else {
        var4 = -1;
    }
    return var4;
...
private void d() {
    ...
    this.a1002.fillRect(15, 13, 210, 297);
    for (int i = 0; i < 10; i++) {
        this.a1002.drawImage(this.a1013[3][25], 15, (i << 5) + 13, 0);
        this.a(this.a1013[3][25], 216, (i << 5) + 13, 1);
    }
}
...
private void d(int var1) {
    for (int i = 0; i < 3; i++) {
        this.a1002.drawImage(this.a1013[4][6], this.u + i * 176, this.v + var1, 0);
    }
}
...
//
//    数据结构:
//    - c1107[var1] = 敌人数据: [2]=路径ID, [3]=敌人等级
//    - f1106[路径ID] = 路径是否已验证标志
//    - r1101 = 敌人HP数据
//    - this.ac = 阵营选择 (0或1)
//    - this.o = 失败原因代码 (0=HP验证失败, 4=等级过高, 5=科技树未解锁, 6=阵营路径未解锁)
//
//    验证逻辑:
//    1. 路径已验证 (f1106[路径ID]==true):
//       - 等级<5: 验证HP j(a(路径ID,r1101,等级)), 通过返回true, 失败o=0
//       - 等级>=5: o=4, 返回false
//    2. 路径未验证:
//       - 等级<5: 验证HP (不设置f1106), 通过返回true, 失败o=0
//       - 等级==5:
//         * 路径ID==0: 检查科技树e(), 通过则验证HP并设置f1106, 失败o=0; 科技树未通过o=5
//         * 路径ID!=0: 根据阵营ac分支:
...
private boolean d(int var1) {
    ...
    if (!pathVerified) {
        ...
        if (level < LEVEL_5) {
            ...
            int hp = this.a(var6[IDX_PATH], this.r1101, var6[IDX_LEVEL]);
            if (this.j(hp)) {
                return true;
            }
            this.o = 0;
            ...
            if (level == LEVEL_5) {
                ...
                if (var6[IDX_PATH] == 0) {
                    ...
                    if (this.e()) {
                        ...
                        int hp = this.a(var6[IDX_PATH], this.r1101, var6[IDX_LEVEL]);
                        if (this.j(hp)) {
                            this.f1106[var6[IDX_PATH]] = true;
                            return true;
                        }
                        this.o = 0;
                    } else {
                        this.o = LEVEL_5;  // o=5, 科技树未解锁
                    }
                } else {
                    // 路径ID != 0: 根据阵营分支
                    switch (this.ac) {
...
// boolean e() — 科技树全解锁检查 (VF L19432-19455)
private boolean e() {
    for (int i = 0; i < 5; i++) {
        if (!this.a1056[i]) return false;
    }
    return true;
}
...
//
//    数据结构:
//    - c1107[var1] = 敌人数据: [0]=x, [1]=y, [2]=路径ID, [3]=等级, [4]=状态, [6]=HP, [8]=目标, [14]=攻击计数
//    - o1098[路径ID] = 路径偏移量 (<<3 用于坐标计算)
//    - t1103 = 路径数据
//    - b1066 = 塔数据数组
//    - o1153 = 活跃塔索引数组
//    - u1104 = 塔HP数据
...
private boolean e(int var1) {
    ...
    byte TRUE = 1;
    byte IDX_PATH = 2;
    int[][] enemies = this.c1107;
    int[] var7 = enemies[var1];

    // 计算敌人当前坐标 (含路径偏移)
    int x = var7[0] + (this.o1098[var7[IDX_PATH]] << 3);
    int y = var7[TRUE] + (this.o1098[var7[IDX_PATH]] << 3);
    int attackRange = this.a(var7[IDX_PATH], this.t1103, var7[IDX_LEVEL]);
    ...
    switch (var7[IDX_PATH]) {
        ...
        case 7:
            ...
            for (int i = 0; i < this.aP; i++) {
                int[] tower = this.b1066[this.o1153[i]];
                // 检查塔是否在攻击范围内
                if (this.a(tower[0], tower[TRUE], x, y, attackRange)) {
                    // 检查塔类型
                    if (f(tower[8])) {
                        // 计算对塔的伤害
                        int dmg = this.a(var7[IDX_PATH], this.u1104, var7[IDX_LEVEL]);
        ...
        case 6:
            // ========== 直接攻击 ==========
            if (this.a(var7[0], var7[TRUE], var7[5], false)) {
                int dmg = this.a(var7[IDX_PATH], this.u1104, var7[IDX_LEVEL]);
                var7[TOWER_HP] = dmg;
                this.v(var1);
            }
        ...
        case 9:
            // ========== 范围攻击+debuff ==========
            if (this.p(var7[IDX_PATH], var7[IDX_LEVEL])) {
                // 对范围内所有塔施加debuff
                for (int i = 0; i < this.aP; i++) {
                    int[] tower = this.b1066[i];
                    if (this.a(tower[0], tower[TRUE], x, y, attackRange)) {
                        if (f(tower[8])) {
            ...
            // 然后直接攻击
            if (this.a(var7[0], var7[TRUE], var7[5], true)) {
                int dmg = this.a(var7[IDX_PATH], this.u1104, var7[IDX_LEVEL]);
                var7[TOWER_HP] = dmg;
                this.v(var1);
            }
...
private boolean e(int var1, int var2) {
    int var3 = this.a(var1, var2);
    return this.E1163[var3] == 8;
}

// void f() — ★ 进度条渲染 (VF L19628-19681)
//    非 v1 标注的 updateEffects!
private void f() {
    if (this.a1007 > this.b1008) {
        int ratio = this.a1007 / this.b1008;
        this.p = ratio > 10 ? 30 : 25;
        int x = 240 - this.p >> 1;
        this.a1002.setColor(16580557);
        this.a1002.setClip(0, 0, 240, 320);
        this.a1002.fillRect(x, 310, this.p, 10);
        // ... 进度条图标绘制 (VF L19628)
    }
}

// boolean f() — 初始化 (VF L20430-20438)
private boolean f() {
    this.aw();
    this.au();
    this.E1179 = true;
...
private boolean f(int var1, int var2) {
    int var3 = this.a(var1, var2);
    return this.E1163[var3] == 10;
}

// void g() — ★ 音量控制 (VF L20471-20518)
...
// boolean g() — 网络上传 (VF L21114-21354)
private boolean g() {
    int var2 = this.b();
    switch (var2) {
        case -1:
...
// void h() — 结局文字渲染 (VF L21381-21414)
private void h() {
    this.a1002.setClip(0, 0, 240, 320);
    this.a1002.setColor(16777215);
    this.a1002.fillRect(0, 0, 240, 320);
    int var4 = this.b1015[104].length();
    var4 *= this.k;
    var4 = 240 - var4 >> 1;
    int var5 = 320 - this.j >> 1;
    this.a1002.setColor(14834304);
    this.a1002.drawString(this.b1015[104], var4, var5, 0);
    // ... (VF L21381)
}

// void i() — 输入处理 (VF L21989-22009)
private void i() {
    switch (this.h1047) {
        case -7:
            this.l1031 = false;
...
// void j() — 渲染 (VF L22265-22347)
private void j() {
    this.a1002.setColor(16580557);
    // ... (VF L22265)
}

// void k() — 初始化随机数 (VF L22470-22492)
private void k() {
    this.e(0);
    this.a1012 = new Random();
    this.a1012.setSeed(System.currentTimeMillis());
    this.b1018 = true;
    this.e(5);
    this.s = 0;
    this.c1020 = false;
...
// void n() — ★ 滚动位置更新 (VF L23551-23591)
//    非 v1 标注的 drawTowers!
private void n() {
    this.u -= 2;
    if (this.u < -176) this.u = 0;
    this.w -= 2;
    if (this.w < -176) this.w = 0;
    // ... (VF L23551)
}
...
// void s() — 存档面板 (VF L24545-24631)
private void s() {
    this.b(this.ad, 13);
    this.a(this.a1013[3][4], 0, 0, 13450878, true);
    // ... (VF L24545)
}

// void t() — 存档加载 (VF L24708-24925)
private void t() {
    this.n();
    switch (this.ae) {
        case 0:
...
// void u() — ★ 阵营选择状态机 (VF L25041-25232)
...
// void w() — 特殊模式渲染 (VF L25793-25959)
private final void w() {
    this.c(2);
    // ... (VF L25793)
}

// void x() — (VF L26145-26182)
private void x() {
    this.c(1);
    // ... (VF L26145)
}

// void y() — 地图渲染 (VF L26190-26281)
private void y() {
    this.a1002.setColor(16777215);
    this.a1002.fillRect(0, 0, 240, 320);
    int var7 = 0;
    while (var7 < this.a1038) {
        // ... 遍历地图层渲染 (VF L26190)
        var7++;
    }
}

// void z() — ★★ 敌人移动更新 (VF L26684-26857)
//    ★ v11 修正: 完整恢复 (原为部分实现)
//    约173行,敌人移动核心逻辑
//
//    数据结构 a1039[i]:
...
private void z() {
    ...
    for (int i = this.a1038 - 1; i >= 0; i--) {
        int[] var6 = this.a1039[i];
        int mode = var6[2];
        switch (mode) {
            case 1:
                // ========== 直线插值移动 ==========
