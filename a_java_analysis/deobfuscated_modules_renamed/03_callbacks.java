...
//       b. 记录帧开始时间
//       c. 调用 updateMainFrame() — 主帧更新逻辑(状态分发)
//       d. 清除按键状态 keyState=0
...
public final void run() {
    ...
    if (this.threadRunning) {
        // 已有线程:等待并设置状态
        int n = this.waitHttpResponse();           // ★ c() 返回 int, 不是 void!
    ...
    while (true) {
        ...
        try {
            ...
            // 主帧更新 (状态机分发)
            this.updateMainFrame();  // ★ updateMainFrame() 是主帧更新, 不是渲染帮助!
            ...
            if (this.soundEnabled) {
                this.frameTimeStats = (int)(System.currentTimeMillis() - frameStartTime + this.frameTimeStats);
            }
...
public final void keyPressed(int keyCode) {
    ...
    try {
        ...
        if (this.gameState != 46 || keyCode == -5 || keyCode == -7) {
            this.selectedTowerIdx = -1;
        }
        
        this.isTouch = false;
        this.dispatchKeyState();  // 按键状态机分发
    } catch (Exception e) {
        e.printStackTrace();
    }
}
...
public final void hideNotify() {
    ...
    if (this.notifyState == 1500) {
        this.needRestore = true;
    }
    
    // 步骤2: 检查游戏状态, 仅在 l∈{2,12,13,22,23} 时处理隐藏
    // (VF嵌套if结构: l∈{46,48,47}时不进入处理块)
    if (this.gameState != 2 && this.gameState != 12 && this.gameState != 13 
        && this.gameState != 22 && this.gameState != 23) {
        return;
    }
