    ...
    if (var5) {
        ...
        switch (var5) {
            ...
            case 2:
                ...
                this.drawScrollAndBg(); this.renderOverlay(); this.drawSpriteRow(var4, 0);
                break;
            case 3:
                this.renderAK_block297(); this.renderGameWorld(); this.renderAG_block298(); this.drawScrollAndBg();
                this.renderJ(); this.drawSpriteRow(var4, var3);
                break;
            case 4:
            case 20:
                this.renderScrollText(this.gameTexts[var2]);
                if (this.gameState == 20) {
                    int x = 240 - this.spriteImages[4][25].getWidth() >> 1;
                    this.helperC_3P_void(5, x, 0);
                }
                this.drawSpriteRow(var3, var2);
                break;
            case 5:
            case 21:
                this.renderScrollText(this.gameTexts[0]);
                if (this.gameState == 21) {
                    int x = 240 - this.spriteImages[4][25].getWidth() >> 1;
                    this.helperC_3P_void(4, x, 0);
                }
                this.drawSpriteRow(var3, var2);
                break;
            case 8:
                this.renderHelpPage(true);
                this.drawSpriteRow(var4, var3);
                break;
            case 9:
                this.renderHelpPage(false);
                this.drawSpriteRow(var4, var3);
                break;
            case 10:
                this.drawMainMap();
...
// ============================================================================
private void runLoadProcess() {
    this.updateLoadingProgress(0); this.resetCounters(); this.updateLoadingProgress(20); this.loadSprite(4);
    this.updateLoadingProgress(40); this.loadSprite(0); this.updateLoadingProgress(80);
    // ... 完整见 VF L5443
}
    ...
    switch (var6) {
        case 0:
            ...
            if (this.loadingState == 2) {
                ...
                if (this.loadingProgress <= 35) {
                    this.helperF_2P(0, this.loadingProgress);
                }
                // ... 完整见 VF L7309
            }
            break;
...
private void drawBackground() {
    ...
    this.drawImageVariant5P_bool(var1, 0, 0, 13450878, true);
...
private void updateGameState() {
    if (this.victoryFlag) {
        ...
        this.stateS++;
        this.advanceLevel();
        this.nextTurnTarget = this.stateV + 12;
        this.loadSprite(this.nextTurnTarget);
        this.stateU = this.nextTurnTarget;
        // ... 完整见 VF L8518
    }
}
...
private void selectGeneral() {
    ...
    int var3 = this.currentGeneralIdx;
    byte[] var5 = var2[var3];
    var3 = this.generalIdxO;
    this.generalIdxN = var5[var3];
}
...
// ============================================================================
private void checkVictory() {
    if (this.currentTurn == this.targetTurn) {
        if (this.activeTowerCount == 0) {
...
// ============================================================================
private void resetGame() {
    this.helperH_1P(this.currentMidiIdx);
    this.stateAW = 0;
...
private final void updateGameFrame() {
    ...
    this.updateTowerLogic(1);          // 更新塔逻辑
    
    // 胜利条件检查
    if (this.gameState == 2 && this.activeTowerCount <= 0 && this.currentTurn == this.targetTurn && this.currentKeyCode == 35) {
    ...
    int var9 = this.activeTowerCount;
...
private final void renderGameWorld() {
    ...
    int var3 = this.activeTowerCount;
    a(var1, var2, var3, 1);
    // ... 完整见 VF L9590
}
